...
<%
String pNoSeq2 = request.getParameter("pNoSeq2");
%>
<!DOCTYPE html>
<html>
<head>
...
<!-- myPlanView에서 일정공유하기 --> 
	<c:set var="pNoSeq2" value="<%=pNoSeq2%>"/>
	<c:set var="doneLoop" value="false"/>
	<c:if test="${!empty planlist}">
<textarea id="plan" name="plan" cols="100" rows="10" class="form-control mb-2" readonly>일정목록
<c:forEach var="planDate" items="${planlist}"><c:if test="${planDate.pNoSeq2 eq pNoSeq2}"><c:if test="${not doneLoop}">
${planDate.pDay} ${planDate.pPlace}</c:if><c:if test="${planDate.pNoSeq2 ne pNoSeq2}">
<c:set var="doneLoop" value="true"/></c:if></c:if></c:forEach>
</textarea>
	</c:if>
